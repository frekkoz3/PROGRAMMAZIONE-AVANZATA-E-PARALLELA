...
void perf_linked_list_rmv(int size, int repetitions)
{
  linked_list lst = lst_make();
  for (int i=0; i < size; i++) {
    lst_add(lst, rand());
  ...
  clock_t end = clock();
  float ms = (end - start) / (CLOCKS_PER_SEC / 1000.0);
  printf("Removing in a linked list of %d elements %d times: %f ms\n", size, repetitions, ms);
  lst_delete(lst);
}
...
void perf_unrolled_linked_list_rmv(int size, int repetitions)
{
  unrolled_linked_list lst = ulst_make();
  for (int i=0; i < size; i++) {
    ulst_add(lst, rand());
  ...
  clock_t start = clock();
  for (int i=0; i < repetitions; i++) {
    ulst_remove(lst, rand());
  }
  clock_t end = clock();
  float ms = (end - start) / (CLOCKS_PER_SEC / 1000.0);
  printf("Removing in an unrolled linked list of %d elements %d times: %f ms\n", size, repetitions, ms);
  ulst_delete(lst);
}
...
int main(int argc, char * argv[])
{
  ...
  //perf_linked_list_rmv(TEST_LIST_SIZE, 10);
  //perf_unrolled_linked_list_rmv(TEST_LIST_SIZE, 10);
  return 0;
}